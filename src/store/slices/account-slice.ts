...
export const loadAccountDetails = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    if (addresses.TRIM_ADDRESS) {
        const tirmContract = new ethers.Contract(addresses.TRIM_ADDRESS, TrimTokenContract, provider);
        trimBalance = await tirmContract.balanceOf(address);
        claimAllowance = await tirmContract.allowance(address, addresses.PRESALE_ADDRESS);
    }
