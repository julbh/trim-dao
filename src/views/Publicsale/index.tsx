...
function Publicsale() {
    ...
    const onChangePresale = async (action: string) => {
        if (action === "claim" && !claimStart){
            dispatch(info({ text: messages.claim_start }));
            return;
        }
        if (action === "claim" && +claimable === 0){
            dispatch(info({ text: messages.claimable_zero }));
            return;
        }
        if (await checkWrongNetwork()) return;
        if ((quantity === "" || parseFloat(quantity) === 0) && action === "purchase") {
            dispatch(info({ text: action === "purchase" ? messages.before_purchase : messages.before_claim }));
        } else {
